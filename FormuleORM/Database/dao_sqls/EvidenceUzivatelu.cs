...
namespace FormuleSystem.ORM.DAO.Sqls
{
    public class EvidenceUzivatelu
    {
        ...
        public static String SQL_SELECT = "SELECT ID, Login FROM Uzivatel";
        public static String SQL_SELECT_DETAIL_TYMY = "SELECT Uzivatel.ID AS UserID, Type, Tymy.ID AS TymID, Tymy.Nazev, Dodavatel_pneumatik, Staty.ID, Vyrobce_motoru.Nazev FROM Uzivatel JOIN Tymy ON Uzivatel.ID = Tymy.Uzivatel_ID JOIN Vyrobce_motoru ON Tymy.Vyrobce_motoru_ID = Vyrobce_motoru.ID JOIN Staty ON Tymy.Staty_ID = Staty.ID WHERE Uzivatel.ID = @id";
        public static String SQL_SELECT_DETAIL_GP = "SELECT Uzivatel.ID AS UserID, Type, GP.ID AS GPID, GP.Nazev, Datum, Delka_okruhu, Pocet_kol,Staty.Nazev FROM Uzivatel JOIN GP ON Uzivatel.ID = GP.Uzivatel_ID JOIN Staty ON GP.Staty_ID = Staty.ID WHERE Uzivatel.ID = @id";
        public static String SQL_SELECT_DETAIL = "SELECT ID, Type FROM Uzivatel WHERE ID = @id";
        ...
        private static Dictionary<string, string> CteniUzivateleTym(SqlDataReader reader)
        {
            ...
            if (reader.Read())
            {
                ...
                uzivatel.Add("Staty", reader.GetInt32(++i).ToString());
